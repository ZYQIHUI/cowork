...
export const MCP_SERVER_PRESETS: Record<string, Omit<MCPServerConfig, 'id' | 'enabled'> & { requiresEnv?: string[]; envDescription?: Record<string, string> }> = {
  ...
  codegraph: {
    ...
    type: 'stdio',
    command: 'npx',
    args: ['-y', 'codegraph-mcp@latest'],
    env: {
      WORKSPACE_DIR: '',
    },
    requiresEnv: [],
    envDescription: {
      WORKSPACE_DIR: 'Project workspace directory for code analysis (optional, defaults to cwd)',
    },
  },
};
...
class MCPConfigStore {
  ...
  /**
   * Auto-configure CodeGraph MCP if not already present.
   * Called during app initialization when a coding workspace is detected.
   */
  ensureCodeGraphConfigured(workspaceDir?: string): void {
    const servers = this.getServers();
    const existing = servers.find((s) => s.name === 'CodeGraph');
    if (existing) return;

    const preset = this.createFromPreset('codegraph', true);
    if (!preset) return;

    if (workspaceDir) {
      preset.cwd = workspaceDir;
      preset.env = { ...preset.env, WORKSPACE_DIR: workspaceDir };
    }

    this.saveServer(preset);
    log('[MCPConfigStore] CodeGraph MCP auto-configured', workspaceDir ? `for ${workspaceDir}` : '');
  }
