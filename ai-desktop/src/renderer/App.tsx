...
const EvalPanel = lazy(() =>
  import('./components/EvalPanel').then((module) => ({ default: module.EvalPanel }))
);
const CProblemPanel = lazy(() =>
  import('./components/CProblemPanel').then((module) => ({ default: module.CProblemPanel }))
);
const RAGPanel = lazy(() =>
  import('./components/RAGPanel').then((module) => ({ default: module.RAGPanel }))
);
...
function App() {
  ...
  const activeToolPanel = useAppStore((s) => s.activeToolPanel);
  const setActiveToolPanel = useAppStore((s) => s.setActiveToolPanel);
    ...
    <div className="h-full w-full min-h-0 flex flex-col overflow-hidden bg-background">
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        <main className="flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden bg-background">
          ...
          ) : activeToolPanel === 'eval' ? (
            <>
              <div className="flex items-center px-4 py-2 border-b border-border-muted bg-surface-muted">
                <button
                  className="px-3 py-1.5 text-xs rounded-md hover:bg-surface-hover transition-colors flex items-center gap-1.5 text-text-secondary"
