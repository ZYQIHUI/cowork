...
export type ClientEvent =
  ...
  | { type: 'eval.runDocumentEval'; payload: Record<string, never> }
  | { type: 'cproblem.parsePdf'; payload: { pdfPath: string } }
  | { type: 'cproblem.generateCode'; payload: { parsedData: unknown; attachmentPaths: string[] } }
  | { type: 'cproblem.runCode'; payload: { code: string } }
  | { type: 'cproblem.generatePaper'; payload: { parsedData: unknown; codeOutput?: string; charts?: { name: string; path: string; type: string }[] } }
  | { type: 'dialog.selectPdf'; payload: Record<string, never> }
  | { type: 'dialog.selectAttachments'; payload: Record<string, never> }
  | { type: 'rag.selectDocument'; payload: Record<string, never> }
  | { type: 'rag.uploadDocument'; payload: { filePath: string } }
  | { type: 'rag.listDocuments'; payload: Record<string, never> }
  | { type: 'rag.deleteDocument'; payload: { docId: string } }
  | { type: 'rag.askQuestion'; payload: { question: string; mode?: 'precise' | 'conversational' } }
  | { type: 'rag.buildWiki'; payload: { docId: string } }
  | { type: 'rag.searchDocuments'; payload: { query: string; topK?: number } }
  | { type: 'skill.listSkills'; payload: Record<string, never> }
  | { type: 'skill.generateDocument'; payload: { skillName: string; input: Record<string, unknown> } }
  | { type: 'skill.runLatex'; payload: { tex: string } }
  | { type: 'skill.runPandoc'; payload: { inputPath: string; outputPath: string; format: string } };
...
export type ServerEvent =
  ...
  | { type: 'cache.statsUpdated'; payload: import('../../main/extensions/cache-monitor-extension').CacheSummary }
