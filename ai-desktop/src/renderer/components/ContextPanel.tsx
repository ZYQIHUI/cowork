...
export function ContextPanel() {
  ...
  // Token usage aggregation (including cache metrics)
  ...
  const tokenUsage = useMemo(() => {
    ...
    const cacheHitRate = input > 0 ? ((cacheRead / input) * 100).toFixed(1) : null;
    return { input, output, cacheRead, cacheWrite, total: input + output, cacheHitRate };
  }, [messages]);

  // Cumulative cache stats from CacheMonitor extension
  const cumulativeCache = useMemo(() => {
    if (!cacheStats) return null;
    const s = cacheStats as Record<string, unknown>;
    const cacheHitRate = typeof s.cacheHitRate === 'number' ? (s.cacheHitRate * 100).toFixed(1) : null;
    return {
      inputTokens: s.inputTokens as number || 0,
      outputTokens: s.outputTokens as number || 0,
      cacheReadTokens: s.cacheReadTokens as number || 0,
      cacheWriteTokens: s.cacheWriteTokens as number || 0,
      cacheHitRate,
      estimatedSavings: s.estimatedSavings as number || 0,
      sessionCount: s.sessionCount as number || 0,
      prefixDrifts: (s.prefixDrifts as unknown[] || []).length,
    };
  }, [cacheStats]);
  ...
  return (
    <div className="w-72 bg-background border-l border-border-muted flex flex-col overflow-hidden text-sm">
      ...
      {activeSession && (
        <div className="px-4 py-3 border-b border-border-muted space-y-1.5">
          ...
          <div className="flex items-center gap-3 text-xs text-text-muted pl-5">
            ...
            {tokenUsage.total > 0 && (
              <span className="ml-auto text-text-muted/70 text-xs flex flex-col items-end gap-0.5">
                <span>{t('context.inputTokens')} {formatTokenCount(tokenUsage.input)} ·{' '}
                {t('context.outputTokens')} {formatTokenCount(tokenUsage.output)}</span>
                {tokenUsage.cacheHitRate !== null && (
                  <span className="text-green-600 dark:text-green-400">
                    {t('context.cacheHitRate', { rate: tokenUsage.cacheHitRate })}
                  </span>
                )}
              </span>
            )}
      ...
      {/* Cumulative Cache Stats (cross-session) */}
      {cumulativeCache && cumulativeCache.sessionCount > 0 && (
        <div className="px-4 py-2.5 border-b border-border-muted space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-text-muted uppercase tracking-wider flex items-center gap-1.5">
              ...
              {t('context.cumulativeCache')}
            </span>
            <span className="text-xs text-text-muted">
              {cumulativeCache.sessionCount} sessions
            </span>
          </div>
          <div className="flex items-center gap-3 text-xs text-text-muted pl-5">
            <span>{t('context.inputTokens')} {formatTokenCount(cumulativeCache.inputTokens)}</span>
            <span>{t('context.outputTokens')} {formatTokenCount(cumulativeCache.outputTokens)}</span>
            <span className="text-info">Cache {formatTokenCount(cumulativeCache.cacheReadTokens)}</span>
          </div>
          {cumulativeCache.cacheHitRate !== null && (
            <div className="flex items-center gap-2 pl-5">
              <div className="flex-1 h-1.5 bg-surface-muted rounded-full overflow-hidden">
                <div
                  className="h-full bg-green-500 rounded-full transition-all duration-300"
                  style={{ width: `${Math.min(Number(cumulativeCache.cacheHitRate), 100)}%` }}
                />
              </div>
              <span className="text-xs font-medium text-green-600 dark:text-green-400">
                {cumulativeCache.cacheHitRate}%
              </span>
            </div>
          )}
          {cumulativeCache.estimatedSavings > 0 && (
            <div className="text-xs text-text-muted pl-5">
              Est. savings: ${cumulativeCache.estimatedSavings.toFixed(4)}
            </div>
          )}
          {cumulativeCache.prefixDrifts > 0 && (
            <div className="text-xs text-warning pl-5">
              {cumulativeCache.prefixDrifts} prefix drift(s) detected
            </div>
          )}
        </div>
      )}
